...
const setWordAchievedSchema = z.object({
  wordId: z.string().min(1),
  achieved: z.boolean(),
});
...
export async function setWordAchievedAction(
  input: z.infer<typeof setWordAchievedSchema>,
): Promise<ActionResult<null>> {
  if (!hasStudyPrismaModels()) {
    return { ok: false, message: STUDY_PRISMA_HINT };
  }

  const parsed = setWordAchievedSchema.safeParse(input);
  if (!parsed.success) {
    return { ok: false, message: "Invalid achieve payload" };
  }
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const round of rounds) {
        ...
        const isRoundComplete = round.wordIds.every((id) => completed.has(id));
        const nextStatus = isRoundComplete ? "completed" : "active";
    ...
    return {
      ok: false,
      message: error instanceof Error ? error.message : "Failed to update achieved state",
    };
  }
